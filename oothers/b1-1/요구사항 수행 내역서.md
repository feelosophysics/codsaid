...
[2026-05-13 17:23:01] PID:3943 CPU:1.2% MEM:5.3% DISK_USED:1%
[2026-05-13 17:24:02] PID:3943 CPU:1.1% MEM:5.2% DISK_USED:1%
[2026-05-13 17:25:01] PID:3943 CPU:1.4% MEM:4.4% DISK_USED:1%
[2026-05-13 17:26:01] PID:3943 CPU:10.5% MEM:5.4% DISK_USED:1%
[2026-05-13 17:27:02] PID:3943 CPU:1.3% MEM:4.6% DISK_USED:1%
[2026-05-13 17:28:01] PID:3943 CPU:1.0% MEM:4.2% DISK_USED:1%
[2026-05-13 17:29:01] PID:3943 CPU:1.7% MEM:5.6% DISK_USED:1%
[2026-05-13 17:30:02] PID:3943 CPU:1.1% MEM:4.4% DISK_USED:1%
```
...
# 1단계: top -bn1 (전체 보고서 가져오기)
# top: 현재 컴퓨터의 CPU, 메모리 상태를 보여주는 명령어입니다.
# -b (Batch mode): 화면을 계속 새로고침하지 말고, 텍스트 형태로 한 번만 출력하라는 뜻입니다. (스크립트용 옵션)
# -n1 (Number 1): 딱 1번만 실행하고 종료하라는 뜻입니다.

# 2단계: grep "Cpu(s)" (필요한 줄만 찾기)
# top을 실행하면 수십 줄의 정보가 나오는데, 그중에서 CPU 상태가 적힌 줄만 골라냅니다.
# 결과 예시: %Cpu(s): 5.0 us, 2.0 sy, 0.0 ni, **93.0 id**, 0.0 wa...
# 여기서 우리가 필요한 건 93.0 id 부분의 숫자입니다.

# 3단계: sed -n 's/.*, *\([0-9.]*\) *id.*/\1/p' (숫자만 도려내기)
# 이 부분이 가장 복잡한 '외과 수술' 단계입니다. **sed**라는 도구를 써서 글자들 사이에서 숫자만 뽑아냅니다.
# s/찾을패턴/바꿀내용/: 특정 패턴을 찾아서 바꾼다는 뜻입니다.
# .*,: 콤마(,) 앞에 있는 모든 글자들을 의미합니다.
# \([0-9.]*\): 핵심입니다! 숫자(0-9)와 점(.)으로 이루어진 부분을 **'기억'**해두라는 뜻입니다. (이게 바로 Idle 값입니다.)
# *id.*: 숫자 뒤에 오는 id라는 글자와 그 뒤의 모든 것들을 의미합니다.
# \1: 위에서 기억해둔 첫 번째 내용(숫자)만 남기고 나머지는 다 지워버리라는 뜻입니다.
# -n과 p: 바뀐 그 결과물만 화면에 출력(print)하라는 뜻입니다.
# 결과: %Cpu(s): ... 93.0 id ... 전체 문장이 사라지고 **93.0**이라는 숫자만 남게 됩니다.

# 요약: "top 보고서에서 CPU 정보를 찾은 뒤, id 글자 앞에 붙어있는 순수 숫자만 자석처럼 쫙 끌어당겨서 저장해라!"
```
